...
- Phase 6. Candidate Sourcing Consolidation: completed
  ...
  - bounded fallback remains discovery-only in normal ranking, while a narrow Han salvage path is available only when normal layer-1 coverage and fuzzy salvage are both globally absent
    - cheap continuity layering follow-up (2026-04-13): completed
      ...
      - cheap Han body exact prefetch is now gated by exact-primary competition instead of potential-prefix coverage: in `strict` mode it only cold-prefetches candidates tied with the current exact-primary leader, while `normal` and `off` modes allow candidates within one exact-primary unit of that leader; verification and Han backstop paths remain ungated so this stays a third-layer budget rule rather than a cross-layer ranking rule
  ...
  - proximity witness follow-up (2026-04-13): completed
    ...
    - local witness construction now supports Latin token exact/prefix spans plus Han exact concatenated-token spans, and metadata/body Han exact fallback can reuse normalized field text or doc-level Han exact witness hooks without changing snapshot/schema/index formats
    - V2 runtime storage readers now expose doc-level Han exact witness lookup by composing existing logical-block metadata with cached cold Han exact payload, and the verification hydration path prefetches only the small verification-bucket Han block set under the existing cold-budget limits
    - focused evidence, comparator-signal, candidate-cascade, independent-engine, and index-store regression coverage now guards prefix-participating proximity, contiguous Han token spans, and restored doc-level Han witness reuse

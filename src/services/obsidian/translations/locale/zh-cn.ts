export default {
	...
	"Database has been updated, a reindex is required": "数据库版本已更新，需要重建索引",
	"Hybrid search": "混合搜索",
	"Hybrid search desc": "混合词法搜索与千问语义搜索。当 embedding 或 rerank 链路失败时，会自动降级为词法结果。",
	Manage: "管理",
	Update: "更新",
	Add: "添加",
	Enable: "启用",
	"Enter path...": "输入路径...",
	"hybridModal.desc": "混合搜索结合了词法搜索与千问语义检索。索引阶段使用 text-embedding-v4，搜索阶段使用 qwen3-rerank，接口返回的 token 用量会计入每周限额。当前实现按小块索引、按文件分组展示。",
	"hybridModal.apiDomain": "API 域名",
	"hybridModal.apiDomain.desc": "留空默认使用 dashscope.aliyuncs.com。若使用代理，只填写域名即可。",
	"hybridModal.apiKey": "API Key",
	"hybridModal.apiKeyNotice": "这里显示的 token 以千问接口返回的 usage 为准。当前混合搜索会使用 text-embedding-v4 和 qwen3-rerank，请确认 API Key 已开通这两个模型。",
	"hybridModal.weeklyTokenLimit": "每周 token 限额",
	"hybridModal.weeklyTokenLimit.desc": "每周（周一至周日）最多允许消耗的 token 数量，设为 0 表示不限制。",
	...
	"hybridModal.weeklyLimitExceededNotice": "本周 token 用量已超过限制，剩余额度为 0。",
	"hybridModal.indexConcurrency": "索引并发数",
	"hybridModal.indexConcurrency.desc": "混合索引时并行调用 embedding 的文件数量。更高的值可能更快，但也更容易触发限流、超时或增加 UI 压力。",
	"hybridModal.vectorCompression": "向量量化",
	"hybridModal.vectorCompression.desc": "选择混合搜索向量的量化与存储方式。Int8 占用更小、搜索更快；Float16 占用更大，但在纯语义匹配时通常更稳。修改后需要重建 hybrid 索引。",
	...
	"hybridModal.fileRankStrategy.desc": "同一文件命中多个小块时，如何合并成文件级排序分数。",

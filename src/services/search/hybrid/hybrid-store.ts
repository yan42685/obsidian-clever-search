...
export function bm25ToBlob(index: BM25Index): Blob {
  ...
  const termEntries = Object.entries(index.termDict).sort((a, b) =>
    compareTerms(a[0], b[0]),
  );
...
export async function getBm25BlobVersion(
  blob: Blob,
): Promise<2 | 3 | 4 | 5 | null> {
...
function analyzeBm25BlobV3(
  ...
  let headerBytes =
    reader.position - docCountStart + BM25_BINARY_MAGIC_V3.length;
  let termTextBytes = 0;
  let termMetaBytes = 0;
...
function analyzeBm25BlobV5(
  ...
  let headerBytes =
    reader.position - docCountStart + BM25_BINARY_MAGIC_V5.length;
  let termTextBytes = 0;
  let termMetaBytes = 0;
  let postingHeaderBytes = 0;
  let postingDocDeltaBytes = 0;
...
function analyzeBm25BlobV4(
  ...
  let headerBytes =
    reader.position - docCountStart + BM25_BINARY_MAGIC_V4.length;
  let termTextBytes = 0;
  let termMetaBytes = 0;
  let postingHeaderBytes = 0;
  let postingDocDeltaBytes = 0;
...
function analyzeBm25BlobV2(
  ...
  let headerBytes =
    reader.position - docCountStart + BM25_BINARY_MAGIC_V2.length;
  let termTextBytes = 0;
  let termMetaBytes = 0;
  let postingHeaderBytes = 0;
  let postingDocDeltaBytes = 0;
...
function writeVarUint(value: number): Uint8Array {
  if (!Number.isInteger(value) || value < 0) {
    throw new Error(
      `VarUint only supports non-negative integers, got ${value}`,
    );
  }
  const bytes: number[] = [];
  let current = value;
...
class BinaryReader {
  ...
  readString(byteLength: number): string {
    ...
    return textDecoder.decode(
      new Uint8Array(this.view.buffer, start, byteLength),
    );
  }
}

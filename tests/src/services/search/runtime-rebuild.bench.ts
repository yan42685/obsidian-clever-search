...
describe("runtime rebuild benchmark", () => {
  ...
  test("prints local-only rebuild timings", async () => {
    ...
    } = require("src/services/search/hybrid/chunker");
    ...
    await recordStage(
      ...
      async () => {
        ...
        for (let index = 0; index < shardRows.length; index++) {
          const restoredVectors = await rowToChunkVectorRecords(
            shardRows[index],
          );
...
function analyzeSyntheticConsistency(
  files: SyntheticFile[],
  preparedChunks: PreparedChunk[],
  analyzeHybridStoredFileConsistency: (input: any) => {
...
function sumBlobBytes(
  rows: Array<{ chunkIds: Blob; vectorData: Blob; scaleData?: Blob }>,
): number {
  return rows.reduce(
    (total, row) =>
      total +
